...
const MemoryAdmin = () => {
  ...
  const getImageUrl = (src) => {
    ...
    // case: '/uploads/filename.jpg' => VITE_VITE_API_URL + src
    if (src.startsWith("/uploads")) return `${VITE_VITE_API_URL}${src}`;
    // case: '/filename.jpg' (backend sometimes saves '/filename') => VITE_VITE_API_URL + '/uploads/filename'
    if (src.startsWith("/")) {
      const filename = src.replace(/^\//, "");
      return `${VITE_VITE_API_URL}/uploads/${filename}`;
    }
    ...
    const fetchData = async () => {
      try {
        const [itemsRes, artistsRes, exhibitionsRes] = await Promise.all([
          axios.get(`${VITE_VITE_API_URL}/api/memory-items`),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${VITE_VITE_API_URL}/api/memory-items/${editingId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Memory item updated!");
      } else {
        await axios.post(`${VITE_VITE_API_URL}/api/memory-items`, formData, {
  headers: { "Content-Type": "multipart/form-data" },
});
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${VITE_VITE_API_URL}/api/memory-items/${deleteTarget}`);
